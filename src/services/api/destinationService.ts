...
import { Destination, DestinationDto, Wishlist, WishlistDto } from "@/types/destination.types";
 ...
 */
export const createDestination = async (data: DestinationDto): Promise<Destination> => {
    console.log("[Destination Service] Creating new destination:", data);
    const { data: newDestination, error } = await apiClient.from("destination").insert([data]).select().single();
    if (error) {
        console.error("[Destination Service] Error creating destination:", error);
        throw error;
    }
    console.log("[Destination Service] Successfully created destination:", newDestination);
 ...
 */
export const updateDestination = async (id: number, data: DestinationDto): Promise<Destination> => {
    console.log("[Destination Service] Updating destination:", data);
    const { data: updatedDestination, error } = await apiClient.from("destination").update(data).eq("id", id).select().single();
    if (error) {
        console.error("[Destination Service] Error updating destination:", error);
        throw error;
    }
    console.log("[Destination Service] Successfully updated destination:", updatedDestination);
...
export const addDestinationToCurrentUserWishlist = async (
    ...
    console.log("[Destination Service] Adding destination to wishlist:", destinationId);
    ...
    console.log("[Destination Service] Destination added to wishlist:", wishlist.id);
    return wishlist;
};
...
export const removeDestinationFromCurrentUserWishlist = async (
    ...
    console.log("[Wishlist Service] Removing destination from wishlist:", destinationId);
    ...
    console.log("[Wishlist Service] Destination removed from wishlist:", destinationId);
};
